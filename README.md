...
```bash
pnpm dlx expo prebuild --platform android --clean   # android/ フォルダ生成
pnpm dlx expo run:android                           # build + install + Metro 起動
```

2 回目以降は `pnpm dlx expo start --dev-client` だけで OK (Metro のみ起動)。

## 主要コマンド

| コマンド | 内容 |
|---|---|
| `pnpm dlx expo start --dev-client` | Metro Bundler 起動 (dev build モード、Expo Go 不可) |
| `pnpm dlx expo run:android` | Android ネイティブビルド + インストール + Metro |
| `pnpm dlx expo prebuild --platform android --clean` | `android/` フォルダ再生成 |
...
| エラー | 原因 | 対処 |
|---|---|---|
| `RNCClipboard could not be found` | Expo Go で起動した (native module 非対応) | `pnpm dlx expo start --dev-client` を使う |
